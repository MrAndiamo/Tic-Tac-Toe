    ...
    if(document.getElementById('userWon').classList.contains('hidden') && document.getElementById('pcWon').classList.contains('hidden')) {
        ...
        if(!document.getElementById('ttt-position-' + id).classList.contains('userChosen') 
            ...
            if(chosenTotal.length != 9 && checkUserWinnings != true) {
                ...
                // Let PC choose from leftover choices
                var pcChoiceRandom = -1;
                while(pcChoiceRandom < 0) {
                ...
                document.getElementById('ttt-position-' + pcChoiceRandom).classList.add('pcChosen');
            }
...
function checkWinnings(chosen) {
    ...
    if(chosen != null) {
        ...
        if(chosen.length > 2) {
            ...
            for(winCheckCounter=0; winCheckCounter < winningCombinations.length; winCheckCounter++) {
            
                if(chosen.indexOf(winningCombinations[winCheckCounter][0]) != -1 && chosen.indexOf(winningCombinations[winCheckCounter][1]) != -1 && chosen.indexOf(winningCombinations[winCheckCounter][2]) != -1){
                    win = true;
                }
